...
export const MovieItem: MessageFns<MovieItem> = {
  encode(message: MovieItem, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    if (message.id !== 0) {
  ...
  decode(input: BinaryReader | Uint8Array, length?: number): MovieItem {
...
export const MovieRequest: MessageFns<MovieRequest> = {
  encode(_: MovieRequest, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): MovieRequest {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
...
export const MovieResponse: MessageFns<MovieResponse> = {
  encode(message: MovieResponse, writer: BinaryWriter = new BinaryWriter()): BinaryWriter {
    for (const v of message.movies) {
      MovieItem.encode(v!, writer.uint32(10).fork()).join();
    }
    return writer;
  },

  decode(input: BinaryReader | Uint8Array, length?: number): MovieResponse {
    const reader = input instanceof BinaryReader ? input : new BinaryReader(input);
    let end = length === undefined ? reader.len : reader.pos + length;
...
export interface Movie {
  GetMovies(request: DeepPartial<MovieRequest>, metadata?: grpc.Metadata): Promise<MovieResponse>;
}
...
export class MovieClientImpl implements Movie {
  ...
  GetMovies(request: DeepPartial<MovieRequest>, metadata?: grpc.Metadata): Promise<MovieResponse> {
    return this.rpc.unary(MovieGetMoviesDesc, MovieRequest.fromPartial(request), metadata);
  }
}

export const MovieDesc = { serviceName: "movie.Movie" };

export const MovieGetMoviesDesc: UnaryMethodDefinitionish = {
  ...
  requestType: {
    serializeBinary() {
      return MovieRequest.encode(this).finish();
    },
  } as any,
  responseType: {
    deserializeBinary(data: Uint8Array) {
      const value = MovieResponse.decode(data);
      return {
        ...value,
        toObject() {
          return value;
